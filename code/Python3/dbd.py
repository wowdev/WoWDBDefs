...
class build_version_range (Grammar):
  ...
  def grammar_elem_init(self, sessiondata):
    lhs = self.elements[0]
    rhs = self.elements[1][1] if self.elements[1] else lhs
    if str(lhs.major) != str(rhs.major):
      raise Exception("version range with differing major in {} <> {}".format(lhs, rhs))
    if str(lhs.minor) != str(rhs.minor):
      raise Exception("version range with differing minor in {} <> {}".format(lhs, rhs))
    if str(lhs.patch) != str(rhs.patch):
      raise Exception("version range with differing patch in {} <> {}".format(lhs, rhs))

    self.builds = []
    for b in range(int(str(lhs.build)), int(str(rhs.build)) + 1):
      self.builds += [build_version_raw(lhs.major, lhs.minor, lhs.patch, b)]

class definition_BUILD (Grammar):
  grammar = ( L("BUILD"), SPACE
