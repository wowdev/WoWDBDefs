...
namespace DBDefsTest
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			int unverfiedFields = 0;
			foreach (var definition in definitionCache)
			{
				foreach (var columnDefinition in definition.Value.columnDefinitions)
				{
					string name = columnDefinition.Key.ToLower();
					if (name.StartsWith("padding_") || name.StartsWith("field_"))
					{
						unverfiedFields++;

						if (columnDefinition.Value.verified)
						{
							Console.ForegroundColor = ConsoleColor.Yellow;
							Console.WriteLine(definition.Key + "." + columnDefinition.Key + " is verified but is an unknown field/padding.");
							Console.ResetColor();
						}
					}
				}
			}
			Console.WriteLine("Checked " + unverfiedFields + " unknown/padding fields!");
